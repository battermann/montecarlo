...
public class NestedMonteCarloSearch {
	...
	public static <TState, TAction> Pair<Double, ArrayList<TAction>> executeSearch(INmcsState<TState, TAction> state, final int level) {
		...
		while(!state.isTerminalPosition() ){
			...
			for(TAction action : state.findAllLegalActions()) {
				...
				final Pair<Double, ArrayList<TAction>> simulationResult = level <= 1
					...
					: executeSearch(currentState, level - 1);
